...
fromList = foldl (\acc x -> Coursework.insert x acc) Empty
...
insert x (Node n l r)
  | x < n     = balance (Node n (Coursework.insert x l) r)
  | x > n     = balance (Node n l (Coursework.insert x r))
  | otherwise = Node n l r

-- join two Sets together be careful not to introduce duplicates.
union :: (Ord a) => Set a -> Set a -> Set a
union set1 set2 = foldl (\acc x -> Coursework.insert x acc) set1 (toList set2)

-- return, as a Set, the common elements between two Sets
intersection :: (Ord a) => Set a -> Set a -> Set a
intersection set1 set2 = foldl (\acc x -> if member x set1 then Coursework.insert x acc else acc) empty (toList set2)

-- all the elements in *s1* not in *s2*
-- {1,2,3,4} `difference` {3,4} => {1,2}
-- {} `difference` {0} => {}
difference :: (Ord a) => Set a -> Set a -> Set a
difference s1 s2 = foldl (\acc x -> if not (member x s2) then Coursework.insert x acc else acc) empty (toList s1)
...
powerSet (Node x l r) =
  let withoutX = powerSet (Coursework.union l r)
      withX = setmap (Coursework.insert x) withoutX
  in Coursework.union withoutX withX
